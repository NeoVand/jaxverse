...
export const descent: HoodChapter = {
	slug: 'descent',
	lesson: 'Lesson 0 — the gradient, by hand first',
	lede: `A confession to open the course: <em>nothing in this chapter uses jax-js.</em> Two coordinates and a scalar loss need no tensor library, and seeing the algorithms as bare arithmetic once — every moment buffer a named variable — is the best preparation for meeting them again wrapped in <code>grad</code> and <code>jit</code>. The lab closes the loop: the same race, rebuilt on jax-js, where the derivative comes from the library instead of finite differences.`,
	ml: [
		{
