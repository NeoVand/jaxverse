...
export const reward: HoodChapter = {
	slug: 'reward',
	lesson: 'Lesson 6 — knowing when not to use the library',
	...
	for (let t = 0; t < ep.states.length; t++) {
		...
		},
		{
			...
			body: `The double pendulum's policy is the same softmax trick over 33 hand-crafted features instead of a table — and every hard-won lesson lives in the <em>reward and curriculum</em>, not the optimizer: whitened advantages so one lucky episode cannot yank the weights, an entropy bonus so the policy keeps exploring, a spin tax so it cannot cheat by becoming a propeller, and a replay buffer of its own successful deliveries so the catch gets practiced from states it can actually reach. RL's dirty secret, on display: the update rule is the easy part.`,
			code: {
