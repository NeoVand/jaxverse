...
export const latent: HoodChapter = {
	slug: 'latent',
	lesson: 'Lesson 4 — the reparameterization trick, and running half a network',
	lede: `The autoencoder is the same MLP engine again — the hourglass is just a layer list that narrows and widens — but the variational waist earns its own lesson. Sampling is not differentiable; the <em>reparameterization trick</em> makes it so, and it is three lines here. The chapter also breaks the network in half on purpose: the map plots <code>encode(x)</code>, the manifold plots <code>decode(z)</code>, and each half is its own worker op.`,
	ml: [
		{
