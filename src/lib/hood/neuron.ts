...
export const neuron: HoodChapter = {
	slug: 'neuron',
	lesson: 'Lesson 1 — params as a tree, valueAndGrad, jit',
	lede: `This chapter is where jax-js starts doing the training for real. Plate III's curve workshop runs on the book's MLP engine — about four hundred lines in <code>src/lib/nn/</code> that also carry the next three chapters unchanged. Three ideas do all the work: parameters live in a plain object (a <em>pytree</em>), <code>valueAndGrad</code> differentiates the loss with respect to that whole object at once, and <code>jit</code> compiles the step so it stops being JavaScript.`,
	ml: [
		{
...
optState = newOptState;`
		...
		},
		{
			...
			body: `Plate I — the single neuron with three sliders — uses no engine at all: it is <code>v·σ(wx + b)</code> redrawn per frame, because a demo about <em>feeling three knobs</em> should not spend a GPU. Knowing when not to reach for the machinery is part of the lesson, and the reward chapter will make the same choice at larger scale.`
		}
	],
	ui: [
		{
