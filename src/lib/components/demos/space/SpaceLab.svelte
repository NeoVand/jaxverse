<script lang="ts">
	...
	const PROBE_RES = 96;
	const probeGrid = makeProbeGrid(PROBE_RES);
	const grid = makeGridLines(16, 33, 1.1);
	...
	async function boot() {
		phase = 'loading';
		errorMsg = '';
		zoom = hiddenMode === '3d' ? 0.72 : 1; // 3-D starts wide enough to see the cube
		const myGen = ++gen;
		try {
			engine?.dispose();
	...
	function guidedBeats() {
		...
		if (stage === 'flat' && step >= 700 && !ctaVisible) ctaVisible = true;
	...
	async function refresh() {
		...
		let outDim = hDim;
		if (mode === 'pca') {
			const proj = pca2(hAll, n + g, hDim);
			ptsV = proj.slice(0, n * 2);
			gridV = proj.slice(n * 2);
			outDim = 2;
		} else {
			ptsV = hAll.slice(0, n * hDim);
			gridV = hAll.slice(n * hDim);
		}
		// view scale: tanh is bounded; relu/pca must be fit
		let scale = 1;
		if (activation === 'relu' || mode === 'pca') {
			let mx = 1e-6;
	...
	function startPainter() {
		...
		const paint = (t: number) => {
			...
			lastDraw = t;
			if (!reduced && !dragging && snapB?.mode === '3d') yaw += dt * 0.00013;
			drawInput();
			drawHidden(t);
		};
		raf = requestAnimationFrame(paint);
	...
	function drawInput() {
		...
		const E = 1.15;
		const px = (x: number) => ((x + E) / (2 * E)) * W;
		const py = (y: number) => H - ((y + E) / (2 * E)) * H;
	...
	function drawHidden(t: number) {
		...
		const S = snap.scale;
		const E = 1.18 / zoom; // wheel zoom widens or narrows the window
		const d = snap.hDim;

		const toXY = (v: number[]): [number, number, number] => {
			if (snap.mode === '3d') {
				const [x, y, z] = project3(v[0] * S, v[1] * S, v[2] * S, yaw, pitch);
				return [((x + E) / (2 * E)) * W, H - ((y + E) / (2 * E)) * H, z];
			}
			return [((v[0] * S + E) / (2 * E)) * W, H - ((v[1] * S + E) / (2 * E)) * H, 0];
		};
		...
		for (const [start, count] of grid.lines) {
			...
			ctx.globalAlpha = snap.mode === '3d' ? 0.5 : 0.75;
			ctx.stroke();
		}
		ctx.globalAlpha = 1;
		...
		if (snap.mode === '2d') {
			...
			const lim = E / S;
			for (const xx of [-lim, lim]) if (Math.abs(b) > 1e-9) ptsLine.push([xx, (-c - a * xx) / b]);
			for (const yy of [-lim, lim]) if (Math.abs(a) > 1e-9) ptsLine.push([(-c - b * yy) / a, yy]);
			const inBox = ptsLine.filter(
				([x, y]) => Math.abs(x) <= lim * 1.01 && Math.abs(y) <= lim * 1.01
			);
			if (inBox.length >= 2) {
		...
		if (snap.mode === '3d') order.sort((a, b) => coords[a][2] - coords[b][2]);
		for (const i of order) {
			const [x, y, z] = coords[i];
			const dn = snap.mode === '3d' ? (z + 1.6) / 3.2 : 1;
			ctx.beginPath();
			ctx.arc(x, y, snap.mode === '3d' ? 1.9 + 1.5 * dn : 3, 0, Math.PI * 2);
			ctx.globalAlpha = snap.mode === '3d' ? 0.45 + 0.55 * Math.max(0, Math.min(1, dn)) : 1;
			ctx.fillStyle = data.labels[i] === 0 ? tk.accent : tk.warm;
			ctx.fill();
		}
		ctx.globalAlpha = 1;
	...
	function pickPoint(ev: PointerEvent) {
		...
		const E = 1.15;
		const mx = ((ev.clientX - r.left) / r.width) * 2 * E - E;
		const my = -(((ev.clientY - r.top) / r.height) * 2 * E - E);
		let best = -1;
		let bestD = 0.008;
...
<div class="flex flex-col" use:inview={autoload}>
	<!-- controls -->
	<div class="flex flex-wrap items-center gap-x-5 gap-y-3 border-b border-line-soft px-4 py-3">
		...
		{:else}
			{#if variant === 'free'}
				<span class="flex items-center gap-1" role="group" aria-label="Dataset">
				...
				<span class="flex items-center gap-1" role="group" aria-label="Hidden width">
					<span class="eyebrow mr-1">width</span>
					{#each [2, 3, 4, 8] as w (w)}
						<button
							class="chip"
				...
				<span class="flex items-center gap-1" role="group" aria-label="Depth">
					<span class="eyebrow mr-1">depth</span>
					{#each [1, 2, 3] as dd (dd)}
						<button
							class="chip"
				...
				<span class="flex items-center gap-1" role="group" aria-label="Activation">
					{#each ['tanh', 'relu'] as act (act)}
						<button
							class="chip"
							class:chip-on={activation === act}
							onclick={() => {
								activation = act as 'tanh' | 'relu';
								void rebuild();
							}}>{act}</button
						>
					{/each}
			...
			<span class="ml-auto flex items-center gap-2">
				<Btn onclick={() => setTraining(!training)}>
					{#if training}
	...
	{#if phase === 'ready'}
		<!-- twin views -->
		<div class="grid grid-cols-1 gap-px bg-line-soft sm:grid-cols-2">
			...
			<div class="relative bg-surface">
				<span class="eyebrow absolute top-3 left-3 z-10">
					...
					{#if hiddenMode === '3d'}<span class="tracking-normal text-ink-3 normal-case">
						...
						>{/if}
					{#if hiddenMode === '2d'}<span class="tracking-normal text-ink-3 normal-case">
							— scroll to zoom</span
						>{/if}
					{#if hiddenMode === 'pca'}<span class="tracking-normal text-ink-3 normal-case">
							— PCA shadow of {width}-D · scroll to zoom</span
						>{/if}
				</span>
				<canvas
					bind:this={hiddenCanvas}
					class="block aspect-square w-full touch-none"
					class:cursor-grab={hiddenMode === '3d' && !dragging}
				...
				{#if variant === 'guided' && ctaVisible && stage === 'flat'}
					<div class="absolute right-3 bottom-3 z-10">
						<Btn kind="primary" onclick={liftDimension}>Give it a third dimension →</Btn>
					</div>
				{/if}
		...
		>
			<span class="num text-[11.5px]">
				{archLabel} · {paramCount} params · {device}
			</span>
			<span class="num text-[11.5px]">
				step {step} · loss {Number.isFinite(lossNow) ? lossNow.toFixed(3) : '—'} · acc {Number.isFinite(
					accNow
				)
					? (accNow * 100).toFixed(0) + '%'
					: '—'} · {msPerStep.toFixed(0)} ms/step
			</span>
			<span class="ml-auto flex items-center gap-4">
				<svg width="120" height="30" aria-label="training loss" role="img">
					<path
						d={sparkPath(lossHist, 120, 30, true)}
						fill="none"
						stroke="var(--accent)"
						stroke-width="1.4"
					/>
				</svg>
				<svg width="120" height="30" aria-label="held-out accuracy" role="img">
					<path
						d={sparkPath(accHist, 120, 30, false)}
						fill="none"
						stroke="var(--warm)"
						stroke-width="1.4"
					/>
